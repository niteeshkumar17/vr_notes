...
```mermaid
flowchart TD
  L["Land + wealth + numbers in one rural pocket"] --> D["Dominant caste"]
  D --> E["Control local economy"]
  D --> P["Pride / desanskritization"]
  D --> M["Politically mobilised"]
  D --> CP["Caste panchayat + boycott → endogamy alive"]
```
