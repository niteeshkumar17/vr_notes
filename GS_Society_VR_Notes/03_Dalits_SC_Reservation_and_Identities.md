...
```mermaid
flowchart TD
  A["Atrocity / welfare gap"] --> B{"Who did it?"}
  B -->|People| C["Police / courts first"]
  B -->|State not listening| D["NCSC Art 338"]
  D --> E["Civil-court powers: summons + directions"]
  E --> F["Not binding — no contempt"]
  F --> G["Annual report → Parliament questions"]
```
...
```mermaid
flowchart TD
  N["Naming: Depressed → Harijan → SC 1935"] --> L["Art 341 President list / 1950 Order"]
  L --> R["330 LS / 332 SLA / 16(4) jobs / 15(4) HEI / 17"]
  R --> C["NCSC 338: safety vs State + welfare advice"]
  C --> NB["Not binding / no contempt / annual report"]
  G["Goal: abolish UT from caste — not caste"] --> P1["1800–1900 reformers: too many agendas"]
  G --> P2["1900–56 Gandhi vs Ambedkar"]
  G --> P3["1970–80 Dalit Panthers MH"]
  G --> P4["1980– BSP 1984: power + keep quota — only success"]
  P2 --> PP["1932 Poona Pact: SE withdrawn"]
  P2 --> NV["1956 Navayana / 5 lakh Nagpur"]
  NV --> CV["Islam/Christian lose SC; Sikh/Buddhist keep"]
  P4 --> H["Haves ~10% hijack quota / have-nots ~90%"]
  H --> CL["Now: creamy layer inside SC + subclass"]
  CL --> DS["2004 homogeneous ban → 2024 Davinder Singh"]
```
