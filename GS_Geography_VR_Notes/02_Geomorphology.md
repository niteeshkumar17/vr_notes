...
```mermaid
flowchart LR
  M1["Model 1 composition"] --> CD["Continental Drift"]
  M2["Model 2 density"] --> SFS["Sea Floor Spreading"]
  M3["Model 3 state of matter"] --> PT["Plate Tectonics"]
```
...
```mermaid
flowchart TD
  IC["Inner core ~6000°C<br/>solid metal + primordial radioactivity"] -->|intense conduction| OC["Outer core ~6000°C<br/>liquid metal + own radioactivity"]
  OC -->|thermal convective cells| LM["Lower mantle<br/>slow conduction — SIMA / silica is a poor conductor"]
  LM -->|slow; ~2200 km; millions of years| AS["Asthenosphere T > 2500°C<br/>own radioactivity + heat added from below"]
  AS -->|thermal convective cells| PL["Plates move"]
```
...
```mermaid
graph TD
  SM["Seismic model 1960s<br/>state of matter"] --> LIT["Lithosphere solid ~100 km<br/>= plates"]
  SM --> AST["Asthenosphere semi-solid<br/>in upper mantle"]
  LIT --> PT["Plate = lithosphere segment<br/>Tectonics = move + interact"]
  AST --> CONV["Non-uniform radioactivity<br/>→ unique convection"]
  CONV --> PT
  OC["Outer-core convective cells"] --> HS["Mantle hotspot<br/>lower mantle"]
  HS --> PLUME["Mantle plume<br/>not all reach AST"]
  PLUME --> CONV
  PT --> BD["Boundary = direction<br/>Margin = O-O / O-C / C-C"]
  BD --> SFS["O-O divergent → MOR<br/>constructive"]
  SFS --> EV["Ridge + age ↑ to continent + symmetry"]
```
...
```mermaid
graph TD
    A["Geomorphology"] --> B["Scientific study of diverse landforms"]
    B --> C["Principle 1: Complexity > Simplicity"]
    B --> D["Principle 2: LF = f(Endo, Exo)"]
    B --> E["Principle 3: Davisian Trio"]

    D --> F["Endogenic Force"]
    D --> G["Exogenic Force"]

    F --> F1["Energy from Interior"]
    F --> F2["Creates Irregularity"]
    F --> F3["Crustal Instability"]
    F3 --> H["Himalayas"]

    F --> EF["Expression by Format"]
    EF --> EF1["Structural: LF (Mountains, Plateaus)"]
    EF --> EF2["Non-Structural: EQ, Volcano, Tsunami"]

    F --> ED["Expression by Duration"]
    ED --> ED1["Diastrophic: Slow (Mountain Building, Plate Tectonics)"]
    ED --> ED2["Catastrophic: Sudden (Earthquake, Volcanic Eruption)"]

    G --> G1["Energy from Insolation"]
    G --> G2["Degradation / Levelling"]
    G --> G3["Crustal Stability"]
    G3 --> I["Aravalis"]

    F1 --> SRC["Source: Geothermal/Primordial Heat at Core"]
    SRC --> DM["Driving Mechanism: Convection"]
    DM --> EXP["Expression: Plate Tectonics, Volcanism, Earthquakes"]

    SRC --> RA["Radioactive Decay: U, Th, Ra"]
    RA --> DA["Density Adjustment in Hadean"]
    DA --> LAYERS["SIAL → SIMA → NiFe"]

    DM --> DD["Density Differentiation"]
    DM --> TD["Thermal Differentiation"]
    DM --> HET["Heterogeneous Interior"]

    LAYERS --> M1["Model 1: Suess 1880s (Composition)"]
    LAYERS --> M2["Model 2: Mohorovičić 1930s (Density)"]
    LAYERS --> M3["Model 3: Seismic (State of Matter)"]

    M1 --> CD["Continental Drift 1912"]
    M2 --> SFS["Sea Floor Spreading 1962"]
    M3 --> PT["Plate Tectonics 1967"]

    DA --> GI["Giant Impact: Theia (4.5 BY)"]
    GI --> MOON["Moon Formation"]
    GI --> TILT["Earth's Axial Tilt (23.5°)"]
    TILT --> WOB["Wobbling → Climate Change"]
```
