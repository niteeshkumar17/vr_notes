...
```mermaid
flowchart LR
  UCT["UCT to women"] --> ERR["Inclusion + exclusion errors"]
  ERR --> POL["Political cost"]
  UCT --> FIS["Deficit / switch from capex"]
  POL --> CCT["CCT / Midday Meal-type link"]
```
...
```mermaid
flowchart TD
  FRA["FRA 2006 · MoTA nodal"] --> TEXT["Act: no project-consent clause — MoTA"]
  RULES["Forest-clearance rules"] --> DONE["Finish FRA process before diversion certificate"]
  TEXT --> FIGHT["Power Ministry vs MoTA"]
  DONE --> FIGHT
...
```mermaid
flowchart LR
  DOG["Vaccinate dogs"] --> SRC["Cut source"]
  PEP["PEP: wash + vaccine + RIG"] --> HUM["Save the bitten"]
  SRC --> OK["Bhutan: WHO-validated"]
  HUM --> OK
```
