...
```mermaid
flowchart LR
    A["India"] --> B["Tashkent, Uzbekistan<br/>bilateral + memorials"]
    B --> C["Bishkek, Kyrgyz Republic<br/>26th SCO Summit"]
    C --> D["6th World Nomad Games<br/>opening ceremony"]
```
...
```mermaid
flowchart TD
    A["Indian vulture crash"] --> B["Diclofenac in cattle carcasses<br/>kidney failure — the classic UPSC story"]
    A --> C["This clip: electrocution<br/>dump too close to high-tension lines"]
    C --> D["Move the dump, not the birds"]
    D --> E["April 2015 relocation<br/>zero recorded deaths after"]
```

<span style="color: #e53e3e;">**Prelims trap:** This clip is **not** about banning **diclofenac** (that remains the *national* collapse story for *Gyps* vultures). Here the fix is **siting**: keep carcass dumps away from **high-tension** infrastructure. **WII** is in **Dehradun, Uttarakhand**.</span>
...
```mermaid
flowchart LR
    A["Factual world<br/>CO₂ over 420 ppm"] --> C["Compare intensity<br/>and probability"]
    B["Counterfactual world<br/>pre-industrial ~280 ppm"] --> C
    C --> D["Human fingerprint?<br/>confidence varies by event type"]
```
...
<span style="color: #e53e3e;">**Mains line:** Attribution is strongest for heat; weakest for short, local convective storms. Do not write “climate change caused yesterday’s cloudburst” unless a study with stated confidence exists.</span>
