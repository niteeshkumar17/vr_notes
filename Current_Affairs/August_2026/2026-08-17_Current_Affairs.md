...
```mermaid
graph TD
    A["Election Commission of India (Art 324)"] --> B["Chief Electoral Officer (CEO) — State/UT<br/>(Sec 13A, RPA 1950)"]
    B --> C["District Election Officer (DEO)<br/>(Sec 13AA, RPA 1950)"]
    C --> D["Electoral Registration Officer (ERO)<br/>(Sec 13B, RPA 1950)"]
    D --> E["Booth Level Officers (BLOs)<br/>(Door-to-Door House Enumeration)"]
    
    E --> F["Special Intensive Revision (SIR)"]
    F --> G["Enumeration Forms Digitised<br/>(Delhi: ~67% / 97.20 Lakh Electors)"]
    F --> H["Unenumerated / Untraced Electors<br/>(Delhi: ~45 Lakh Electors Likely Excluded)"]
    
    H --> I["ASD List<br/>(Absent, Shifted, Dead)"]
    I --> J["Draft Electoral Roll Published"]
    J --> K["Statutory Claims & Objections Window<br/>(Form 6: Addition | Form 7: Deletion | Form 8: Correction/Shifting)"]
    K --> L["Final Electoral Roll Publication<br/>(Mandatory Rule 21A Inquiry Safeguard)"]
```
...
```mermaid
graph TD
    A["'One Nation, One Election' (ONOE)"] --> B["Constitutional Friction"]
    A --> C["Federal Erosion"]
    A --> D["Democratic Deficits"]
    A --> E["Economic & Governance Realities"]

    B --> B1["Grafts Presidential Fixed Tenures onto<br/>Parliamentary Flexible System (Art 75/164)"]
    B --> B2["Premature Dissolution of Stable States<br/>or Truncated 'Lame-Duck' Assemblies"]

    C --> C1["Homogenisation: National Narratives<br/>Overshadow Local/State Priorities"]
    C --> C2["Multi-Party Pluralism Weakened;<br/>Asymmetrical Federalism Subdued"]

    D --> D1["Destroys Continuous Accountability;<br/>Creates 5-Year Power Voids"]
    D --> D2["Diminishes Electorate's Bargaining Power<br/>in Midterm Elections"]

    E --> E1["Public Election Cost is Minimal (~0.05% GDP);<br/>Campaign Spend = Grassroots Economic Circulation"]
    E --> E2["Model Code of Conduct (MCC) Myth:<br/>MCC Prevents Sops, Does NOT Halt Governance"]
```
