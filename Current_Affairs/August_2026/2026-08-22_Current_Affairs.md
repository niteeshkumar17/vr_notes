...
```mermaid
graph TD
    A["Supreme Court 3-Judge Bench<br/>(CJI Surya Kant, Justice Joymalya Bagchi, Justice V. Mohana)"] --> B["Judicial Assessment of Repealed MGNREGA"]
    A --> C["The Successor Law: VB-G RAM G Act"]
    A --> D["Fiscal & Constitutional Dynamics"]

    B --> B1["'Salutary & Effective Scheme'"]
    B --> B2["'Neither a Freebie nor Exploitation'"]
    B --> B3["Pan-India Rural Social Safety Net"]
    B --> B4["Aruna Roy Petition: Delayed Wages & Statutory Compensation"]

    C --> C1["Viksit Bharat Guarantee for Rozgar & Ajeevika Mission (Gramin)"]
    C --> C2["Days Increased: 100 Days → 125 Days Guaranteed Work"]
    C --> C3["Paradigm Shift: Rights-Based Demand-Driven → Centrally Controlled Allocation"]
    C --> C4["Ground Impact: 50% Decline in Employment Generation"]

    D --> D1["Funding Shift: 90:10 (Centre:State) → 60:40"]
    D --> D2["Three-Fold Fiscal Burden on State Treasuries"]
    D --> D3["Fiscal Constraint: 'States Have No Money'"]
    D --> D4["SC Directive: File Fresh Challenge Against VB-G RAM G Act"]
```
...
```mermaid
graph TD
    A["Right to Work: Constitutional Framework"] --> B["Part III: Fundamental Rights (Justiciable)"]
    A --> C["Part IV: Directive Principles (Non-Justiciable Guide)"]

    B --> B1["Article 21: Right to Life with Dignity (Olga Tellis 1985)"]
    B --> B2["Article 23: Prohibition of Forced Labour / Begar"]
    B --> B3["PUDR v. UOI (1982): Sub-Minimum Wage = Forced Labour"]
    B --> B4["Sanjit Roy (1983): Famine/Scarcity Relief Wage Exemptions Void"]

    C --> C1["Article 41: Right to Work within Limits of Economic Capacity"]
    C --> C2["Article 39(a): Adequate Means of Livelihood for All Citizens"]
    C --> C3["Article 43: Living Wage & Decent Working Conditions"]

    A --> D["Supreme Court 3-Judge Bench Scrutiny (Aug 2026)"]
    D --> D1["Justice Bagchi: Art 41 is a 'Democratic Aspiration', not Part III FR"]
    D --> D2["Adv. Prashant Bhushan: Sub-Minimum Wage Violates Art 21 & Art 23"]
    D --> D3["Economic Reality: Wage Floor vs Employment Elasticity Trade-Off"]
```
