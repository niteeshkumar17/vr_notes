...
```mermaid
flowchart LR
  Pu["Pu-239 fuel · core"] -->|"fission → electricity + n"| El["Electricity"]
  Pu -->|"n absorbed"| Bl["Blanket U-238 fertile"]
  Bl -->|"transmutes"| Pu2["More Pu-239 fissile"]
```
...
```mermaid
flowchart TD
  S1["Stage I PHWR · natural U oxide · D2O"] -->|"closed cycle: U-238 + Pu-239"| S2["Stage II FBR · Pu fuel · Na · no moderator"]
  S2 -->|"U-238 blanket → more Pu"| S2
  S2 -->|"Th-232 blanket → U-233"| S3["Stage III AHWR · U-233 · D2O mod / H2O cool"]
  S3 -->|"Th-232 blanket → more U-233"| S3
  S3 --> SS["Self-sufficient on thorium"]
```
...
```mermaid
flowchart TD
  M["Moderator slows n · graphite / D2O / Be · must not absorb"] --> PuF["Pu-239 split by fast n → no moderator"]
  M --> PHWR["PHWR 21/26 · natural U · D2O pressurised"]
  M --> LWR["LWR: H2O absorbs a little → enrichment"]
  LWR --> BWR["BWR Tarapur MH · boils"]
  LWR --> PWR["PWR Kudankulam TN · 1000 MW · Russia"]
  C["Containment: steel+concrete · Chernobyl lacked"] --> R["Reactor ≠ bomb · 3–5% vs >90%"]
  PuF --> FBR["FBR: more fissile after · Na coolant"]
  PHWR --> S1["Stage I closed cycle"]
  S1 --> FBR
  FBR --> S3["Stage III AHWR BARC · Th → U-233"]
```
